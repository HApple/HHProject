...
+ (NSURLSessionTask *)GET:(NSString *)URL
            ...
            responseCache:(HHHttpRequestCache)responseCache
                  ...
                  failure:(HHHttpRequestFailed)failure {
    //读取缓存
    responseCache!=nil ? responseCache([HHNetworkCache httpCacheForURL:URL parameters:parameters]) : nil;
    
    NSURLSessionTask *sessionTask = [_sessionManager GET:URL parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        
        if (_isOpenLog) {HHLog(@"responseObject = %@",[self jsonToString:responseObject]);}
        [[self allSessionTask] removeObject:task];
        success ? success(responseObject) : nil;
        //对数据进行异步缓存
        responseCache!=nil ? [HHNetworkCache setHttpCache:responseObject URL:URL parameters:parameters] : nil;
        
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        
        if (_isOpenLog) {HHLog(@"error = %@",error);}
        [[self allSessionTask] removeObject:task];
...
+ (NSURLSessionTask *)uploadFileWithURL:(NSString *)URL
                             parameters:(id)parameters
                               ...
                               progress:(HHHttHHrogress)progress
                                success:(HHHttpRequestSuccess)success
                                failure:(HHHttpRequestFailed)failure {
...
@implementation NSDictionary (HH)
