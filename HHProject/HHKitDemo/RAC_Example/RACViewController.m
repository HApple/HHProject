...
- (void)commonlyUsed{
    ...
    RACTupleUnpack(NSNumber *num1,NSNumber *num2,NSNumber *num3) = tuple;// 元组
    // 快速包装成一个元组
...
- (void)racMulticastConnectionUsed{
    //当有多个订阅者时，但是我们只想发送一个信号的时候怎么办，这时就可以用RACMulticastConnection
    
    //普通写法，这样的缺点是：每订阅一次信号就得重新创建并发送请求，这样很不友好
    RACSignal *signal = [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
       
        //disSubcribeblock中的代码都统称为副作用
        //发送请求---比如afn
        NSLog(@"发送请求");
    ...
    //1.发送请求，用一个信号内包装，不管有多少个订阅者，只想发送一次请求
...
- (void)racSignalUsed{
    ...
    /* RACSignal 总结
     一 核心：
        ...
        3.只要有数据改变就会有信号发出
        4.数据发出，并不是信号类发出，信号类不能发送数据 （RACSubscriber发出）
     二 使用方法：
        1.创新信号
        2.订阅信号
     三 实现思路：
...
#pragma mark - RACSubOject
-(IBAction)pushAction:(id)sender {
    
    RACSecondViewController *vc = [[RACSecondViewController alloc] init];
    ...
    /*
     ...
     注意 ：
     RACSubject 和 RACRepalySubject的区别
     RACSubject必须要先订阅信号之后才能发送信号
     RACReplaySubject可以先发送信号后订阅
...
- (void)racCommandUsed{
    /*
     RACCommand : RAC中用于处理事件的类，可以把事件如何处理，事件中的数据如何传递，包装到这个类中，
     它可以很方便的监控事件的执行过程 比如看事件有没有执行完毕
     使用场景： 监听按钮点击 ，网络请求
     */
    ...
    //直接订阅执行命令返回的信号
    //2.执行命令
    RACSignal *signal1 = [command execute:@1]; //这里其实用到的是replaySubject 可以先发送命令再订阅
    //在这里就可以订阅信号了
...
- (void)racBindUsed{
    ...
    RACSignal *bindSignal = [subject bind:^RACStreamBindBlock{
       //block调用时刻：只要绑定信号订阅就会调用，不做什么事情
        return ^RACSignal *(id value, BOOL *stop){
            ...
            //block作用:处理源信号内容
            //value：源信号发送的内容
            value = @3; //如果在这里把value值改了，那么订阅绑定信号的值即x就变了
            NSLog(@"接收到源信号的内容: %@",value);
            //返回信号，不能为nil，如果非要返回空-则　empty 或 alloc init
            return [RACReturnSignal return:value];  // RACReturnSignal -> RACSignal -> RACSteam   RACSubject -> RACSignal
        };
    ...
    //总结
    //bind(绑定)的使用思想和Hook的一样 --> 都是拦截API从而可以对数据进行操作，，而影响返回的数据。
    //发送信号的时候会来到return ^RACSignal *(id value, BOOL *stop){}的block 在这个block里我们可以对数据进行一些操作，那么value = @3打印的value和订阅绑定信号的value就会变了。
}
...
- (void)racFilterUsed{
    ...
    //有时候我们想要过滤一些信号，这时候我们便可以用RAC的过滤方法。过滤方法有好多种，如下
    
    // 跳跃 ：如下,skip传入2 跳过前面两个值
    // 实际用处 ：在实际开发中比如 后台返回的数据前面没几个没用，我们想跳跃过去，便可以用skip
    RACSubject *subject = [RACSubject subject];
    ...
    //distinctUntilChanged 如果当前的值跟上次的值一样，就不会被订阅到
    ...
    [subject sendNext:@1];
    [subject sendNext:@2];
    [subject sendNext:@1];//不会被订阅
    
    
    //take 可以屏蔽一些值，去掉后面几个值 这里take为2，则只拿到前两个值
    RACSubject *subject2 = [RACSubject subject];
    [[subject2 take:2] subscribeNext:^(id x) {
        NSLog(@"%@",x);
    }];
    [subject2 sendNext:@1];
    [subject2 sendNext:@1];
    ...
    //takeLast : 和take用法一样，不过他取的是最后的几个值，如下，则取的是最后两个值
    //注意：takeLast 一定要调用sendCompleted，告诉他发送完成了，这样才能取到最后的几个值
    RACSubject *subject3 = [RACSubject subject];
    [[subject3 takeLast:2] subscribeNext:^(id x) {
        NSLog(@"%@",x);
    }];
    [subject3 sendNext:@1];
    [subject3 sendNext:@1];
    ...
    //takeUntil: 给takeUntil传的是哪个信号，那么当这个信号或sendCompleted,就不能再接受源信号的内容了。
    RACSubject *subject4 = [RACSubject subject];
    RACSubject *subject5 = [RACSubject subject];
    [[subject4 takeUntil:subject5] subscribeNext:^(id x) {
        NSLog(@"%@",x);
    }];
    ...
    [subject4 sendNext:@1];
    [subject4 sendNext:@2];
    [subject5 sendNext:@3];  //1
    //[subject5 sendCompleted]; // 或2
    [subject4 sendNext:@4];
    
    
    //ignore 忽略一些值
    //ignoreValues 表示忽略所有值
    //1.创建信号
    RACSubject *subject6 = [RACSubject subject];
    //2.忽略一些值
    RACSignal *ignoreSignal = [subject6 ignore:@2];
    //3.订阅信号
...
- (void)racMapUsed{
    ...
    // RAC映射在实际开发中作用： 比如我们想要拦截服务器返回的数据，给数据拼接特定的东西或想对数据进行操作，从而更改返回值
    ...
    RACSubject *subject2 = [RACSubject subject];
    //绑定信号
    RACSignal *bindSignal2 = [subject2 flattenMap:^RACStream *(id value) {
        //block 只要源信号发送内容就会调用
        //value 就是源信号发送的内容
        //返回信号用来包装成修改内容的值
    ...
    //方式三
    RACSignal *bindSignal3 = [signalofSignals flattenMap:^RACStream *(id value) {
        //value:就是源信号发送内容
        return value;
    }];
    [bindSignal3 subscribeNext:^(id x) {
        NSLog(@"%@",x);
    }];
    //方式四 也是开发中常用的
...
- (void)racCombineUsed{
    ...
    //zipWith-------------------------------------------------------------------------------------------
    //zipWith把两个信号压缩成一个信号，只有当两个信号同时发出信号内容时，并且把两个信号的内容合并成一个元组，才会触发压缩流的next事件。
    //创建信号A
    RACSubject *signalA = [RACSubject subject];
    //创建信号B
    ...
    //[signalA zipWith:signalB] -- 先是A后是B
    [signalA sendNext:@1];
    [signalB sendNext:@2];
    ...
    RACSignal *signalG = [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
        ...
        [subscriber sendNext:@"concat 上部分数据"];
        [subscriber sendCompleted]; //必须要调用sendCompleted方法！
        return nil;
    }];
    ...
    RACSignal *signalH = [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
        ...
        [subscriber sendNext:@"concat 下部分数据"];
        return nil;
    }];
    ...
    //创建组合信号
    RACSignal *concatSignal = [signalG concat:signalH];
    //订阅组合信号
